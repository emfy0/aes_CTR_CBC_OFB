...
uint8_t **make_blocks_form_string(char *stringinput, int *numberofblocks)
{
    int transformedstringlength = length(stringinput) % 16 == 0 ? length(stringinput) :  ( (length(stringinput) / 16 + 1) * 16 );
    char *stringtransformed = (char *)malloc(sizeof(char) * transformedstringlength);
    int i = 0;
    while (i < length(stringinput))
    {
    ...
    uint8_t **blockaddr = (uint8_t**)malloc(sizeof(uint8_t*) * length(stringtransformed) / 16);
    int blockcounter;
...
{
    char* outputstring = (char*)malloc(sizeof(char) * ammountofblocks * 16);
